...
- (void)initialization {
    ...
    self.rulerBackgroundColor = YYUIColorFromRGB(0xffffff);
...
//                                   cy_selfHeight >= cy_fit(120) ? cy_fit(60) : cy_selfHeight / 2.0);
    ...
    self.curLabelFrame = CGRectMake(cy_selfWidth / 2.0 - 80 / 2.0, cy_selfHeight - 18, 80, 16);
}
...
- (void)scrollToValue:(NSInteger)value animation:(BOOL)flag {
    ...
    if (self.rulerDirection == RulerDirectionHorizontal) {
        [self.rulerBackgroundView setContentOffset:CGPointMake(x, 0)
                                          animated:flag];
    } else if (self.rulerDirection == RulerDirectionVertical) {
        [self.rulerBackgroundView setContentOffset:CGPointMake(0, value / _unitValue * _unitPX)
                                          animated:flag];
    } else {
        NSAssert(NO, @"error");
    }
